...
const LeaderShipBoard = () => {
  const [dropDownItem] = useState<ItemType[]>([
    { id: 0, text: "All Games" },
    { id: 1, text: "CLaimed Games" },
    { id: 2, text: "General Leaderboard" },
  ]);
  ...
  const DropDownComp = ({ text, current, id }: any) => {
    return (
      <div
        className={`outline-none cursor-pointer   rounded-[16px] flex items-center justify-between gap-6 w-full h-[66px]  p-[16px] ${
          !current
      ...
      <div className="backdrop-blur-sm w-full h-fit mt-[96px] mb-16 md:mt-[176px]">
        <div className="relative z-[999]  px-[16px] md:px-14  text-white">
          <div className="flex justify-between md:items-center mb-[64px] flex-col md:flex-row gap-[16px] md:gap-0 relative h-[99px] md:h-[initial]">
            <h2 className="font-Archivo_Regular font-normal tracking-[0.5px] md:text-[40px] text-[20px] leading-[21px] md:leading-normal">
              LEADERBOARD
            </h2>
            <div
              id=""
              className="font-droid  outline-none bg-blue-50 border-blue-main border-[2px] overflow-hidden rounded-[16px] absolute right-0 md:top-[0px] top-10 w-full md:w-fit"
              style={{
