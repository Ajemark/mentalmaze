...
const TestProgress = ({modes}: {modes: ModeType[]}) => {
    ...
    const Progress = ({title, text, link, mode}: ModeType) => {
        ...
        const runFunction = () => {
            ...
            if(mode === "completed" && searchParams.get('title') == link){
                return Dot
            }
        }

        return (
            <div className='flex flex-col items-center steps w-full' onClick={mode==="completed" || mode==="current"?() => setSearchParams({title:link}):() => console.log("eas")}>
