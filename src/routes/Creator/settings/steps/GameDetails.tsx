...
const GameDetails = ({
  ...
  const uploadImage = async (event: any, type: any) => {
    ...
    if (type == "question") setIsUploadingQ(true);
    if (type != "question") setIsUploading(true);
    ...
    setImages(
      "https://mentalmaze-game.infura-ipfs.io/ipfs/" + result.cid.toString()
    );
    ...
    <div className="">
      <div className="px-[16px] md:px-[48px]">
        ...
        <div className="w-full mt-[32px]">
          <p className="text-[14px] md:text-base font-Archivo_Regular leading-[15px] md:leading-[17.41px] text-white mb-[4px] md:mb-[8px]">
            Game's Cover
          </p>
          {!ipfs ? (
            ...
            <form onSubmit={(e) => uploadImage(e, "")}>
                      ...
                      <label htmlFor="game-image">
                        <div className="flex flex-col items-center cursor-pointer text-[32px] md:text-[20px]">
                          ...
                          <p className="font-[300] text-[15px] md:text-[20px] font-Archivo_Regular text-white text-center ">
                            ...
                            <span className="text-yellow-500">{imageText}</span>
                          </p>
                        </div>
                      </label>
                      <input
              ...
              {errorMessage.message != "" && errorMessage.where == "image1" && (
        ...
        <div className="border-solid border-blue-main border-[2px] rounded-[8px] px-[12px] md:px-[32px] mt-[32px] pb-[48px]">
          <div className="">
            <div className="flex  items-center justify-end mt-[32px] font-Archivo_Regular w-full ">
              <div className={`relative flex  `}>
                {numbers.map((num, i) => (
                  <div
                    key={i}
                    className={`relative text-white rounded-[8px] w-[27px] h-[30px] text-[12px] md:text-base md:w-[48px] md:h-[40px] p-[2px] -ml-[15px] border-solid shadow-lg`}
                    style={{
                      background:
                        "linear-gradient( rgba(3, 36, 73, 1), rgba(11, 119, 240, 1))",
                    }}
                  >
                    <button
                      onClick={() => {
              ...
              <button
                onClick={() => {
                  ...
                  if (Object.entries(questions).length != 4) {
                    setErrorMessage({
                      message: "Please Enter All Question Details",
                      where: "add",
                    });
                    return;
                  }
                  for (const object in Object.entries(questions)) {
                    const curObject: any = Object.entries(
                      Object.entries(questions)[object]
                    )[1][1];
                    if (curObject == "") {
            ...
            {errorMessage.message != "" && errorMessage.where == "add" && (
            ...
            <label htmlFor="">
              ...
              <input
                ...
                onChange={(e) => {
                  setQuestions((prev: any) => ({
                    ...prev,
                    gameInstruction: e.target.value,
                  }));
                  setCurQuestion((prev: any) => ({
                    ...prev,
          ...
          <div>
            <p className="leading-[17.41px] font-Archivo_Regular  text-white text-inherit font-normal mt-[48px] mb-[8px]">
              {"Available  Options"}
            </p>
            <div className="flex gap-[16px]">
              {["", "", "", ""].map((_, index: any) => {
                return (
                  <div
                    key={index}
                    className="bg-[inherit] border-blue-main border-[2px] rounded-[8px] border-solid h-[64px] text-white"
                  >
                    <input
                      key={index}
                      type="text"
                      value={
                        curquestion?.options ? curquestion?.options[index] : ""
                      }
          ...
          <div className="flex justify-end mt-[24px] md:mt-0">
            <button
              onClick={() => {
                ...
                setNumbers((prev) =>
                  prev.length > 1 ? prev.slice(0, prev.length - 1) : [1]
                );
              }}
              className="float-left bg-black text-white 
                font-Archivo-Bold font-semibold px-[16px] py-[10px] rounded-[8px] border-solid border-blue-main border-[1px] text-[14px] md:text-base"
            >
              Delete Question
            </button>
          </div>
              ...
              if (Object.entries(data).length == 8) {
                for (const index in Object.entries(data)) {
                  ...
                  if (object[1] == "" || !object[1]) {
                    setErrorMessage({
                      message:
                        "Enter all Details. Did You Add A Question Already?",
                      where: "proceed",
                    });
                    return;
                  }
                ...
                setErrorMessage({
                  message: "Enter all Details. Did You Add A Question Already?",
                  where: "proceed",
                });
            }}
          >
