...
const ChooseANickname = () => {
  ...
  const { signInDetails, token, loading, setLoading, setUserDetails }: any =
  ...
  const createUser = async () => {
    ...
    let raw = JSON.stringify({
      ...
      invitedBy: sessionStorage.getItem("refId") ?? "dev",
    });
    ...
    await fetch(
      `${import.meta.env.VITE_REACT_APP_BASE_URL}/api/user/?address=${address}`,
      requestOptions
    )
      .then((response) => {
    ...
    <>
      ...
      <Animation className="pt-[48px] flex flex-col h-full gap-[100px] px-[29.5px] md:px-0">
        <input
          ...
          className="font-[400] mb-[-20px] text-[15px] md:text-[20px]  leading-[23.61px] py-[24px] px-[40px] text-[#8C8C8C] rounded-[16px] md:rounded-t-[16px] w-full bg-blue-90 md:w-[416px] mx-auto text-center"
        />
