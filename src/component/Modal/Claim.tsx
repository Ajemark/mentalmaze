...
const Claim = () => {
  ...
  const updatePlayer = () => {
    ...
    fetch(
      `${import.meta.env.VITE_REACT_APP_BASE_URL}/api/player/update-claim`,
      requestOptions
    )
  ...
  const updateGame = () => {
    ...
    let requestOptions: RequestInit = {
      method: "PUT",
      body: JSON.stringify({ id: gameAddr.game }),
      headers: myHeaders,
      redirect: "follow",
    };

    fetch(
      `${import.meta.env.VITE_REACT_APP_BASE_URL}/api/game/update-claim`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
  ...
  return (
    <div className="flex flex-col h-full ">
      <Animation className="justify-center text-center gap-[56px] max-w-[497px] mx-auto flex w-full items-center h-full px-[8px] flex-col">
        <img
          src={award}
          className="w-[88px] h-[88px] md:w-[initial] md:h-[initial]"
        />
        ...
        {gameAddr?.creator ? (
          <div>
            ...
            <div>
              <button
                className="rounded-[16px] font-droid w-[218px] h-[56px] md:w-[251px] md:h-[80px] text-[16px] mt-2 md:text-[24px] settingsFormbutton"
                onClick={async () => {
                  setErrorMessage("");
                  if (gameAddr) {
                    try {
                      const tx = await mmContract.creatorClaimReward(
                        gameAddr.address,
                        ERC20
                      );
                      console.log(tx);
              ...
              {errorMessage != "" && (
                <p className="text-red-600 font-bold pt-5 uppercase text-center">
                  {errorMessage}
                </p>
              )}
            </div>
          ...
          <div>
            ...
            <div>
              <button
                className="rounded-[16px] font-droid w-[218px] h-[56px] md:w-[251px] md:h-[80px] text-[16px] md:text-[24px] settingsFormbutton"
                onClick={async () => {
                  setErrorMessage("");
                  if (gameAddr) {
                    try {
                      const tx = await mmContract.claimReward(
                        gameAddr.game,
                        ERC20
                      );
                      console.log(tx);
