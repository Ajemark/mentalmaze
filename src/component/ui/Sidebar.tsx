    ...
    width > 768 ? <div className={`w-[104px] h-[90vh] fixed md:flex flex-col justify-between py-[46px] left-0  z-20  opacity-[0.4000000059604645] items-center  mt-[104px] hidden ${challenger ? "bg-black" : "bg-blue-100"}`}>
      <div className='flex flex-col gap-6 w-full'>
        {[{ image: nav1, link: "/create-game", title: "Create Game" }, { image: nav4, link: '/challenger', title: "Challenger" }, { image: nav2, link: '/leaderboard', title: "leaderboard" }, { image: nav3, link: "/", title: "Games" }].map((src, i) => {
          if (src.title == "Challenger" && userDetails?.role.toLowerCase() != 'judge') {
            return
          }
      ...
      <div className='flex flex-col gap-8 w-full '>
        {[{ image: discord, link: "https://discord.gg/8STEwMEu" }, { image: telegram, link: "https://t.me/official_mentalmaze" }, { image: twitter, link: "https://twitter.com/mazemental?s=11&t=meAljIy1rKjh6LjNVYwIFQ" }].map((src, index) => {
          return <a key={index} className={`w-[72px] mx-auto flex relative justify-center ${index == 0 && "border-solid border-t-[2px] border-t-[#85BCF9] pt-[22px]"}`} href={src.link}>
            <img src={src.image} />
          </a>
        })}
      </div>
