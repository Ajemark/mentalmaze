...
const Input = ({placeholder, title, center}:{placeholder: string, title?:string, center?:boolean}) => {
  return (
    <div className=' w-full  flex flex-col'>
        ...
        <input type="text" placeholder={placeholder} className={`border-none outline-none bg-inherit px-[24px] w-full h-full placeholder:font-[300] placeholder:text-[15px] md:placeholder:text-base placeholder:font-Archivo_Regular ${center?"text-center":"text-left"}`}/>
        </div>
    </div>
  )
}
