...
export const HeaderInput = () => {
  ...
  const { userDetails, setSearchText, searchText }: any =
      ...
      .then((result) => {
        if (result.data) {
          const data = window.btoa(
            JSON.stringify({ data: result.data, text: searchText })
          );
          if (location.href.includes("search"))
            location.href = "/search?d=" + data;
    ...
    <>
      <div className="border-blue-50 rounded-2xl   border-2 flex items-center justify-center gap-6 px-5 font-normal text-sm  font-Archivo_Regular w-full">
        ...
        <input
          ...
          className="bg-[inherit] py-4  outline-none border-0 text-white -ml-2"
        />
      </div>
    </>
  );
...
export const ConnectWalletbtn = ({ clickHandler }: ConnectTyype) => {
  ...
  return (
    <div className="flex">
      <button
        className="hidden md:block rounded-2xl border-2 md:py-4 px-[2px] py-[8px] shrink-0 lg:px-6 font-normal font-Archivo_Regular text-[10px] md:text-[12px] lg:text-[15px] leading-[10px] md:leading-[16.32px] header-button text-[#ffffff]"
...
export const Instruction = () => {
  return (
    <p className="font-[300] text-wb-40 font-Archivo_Regular mt-[4px] md:mt-[8px] leading-[17.41px] text-[14px] md:text-[16px]"></p>
  );
};
