...
const Dashboard = () => {
  ...
  const getAllGames = () => {
    ...
    fetch(
      ...
      }/api/judge?pageNumber=${pgNum}&pageSize=10`,
      requestOptions
    )
      ...
      <div className="relative text-white ">
        <div>
          <div className="flex justify-between mb-[72px] flex-col md:flex-row md:items-center">
            <h2 className="font-droid text-[32px] hidden md:block">
              DASHBOARD{" "}
            </h2>
            <h2 className="font-droid text-[16px] md:text-[32px] md:hidden">
              DASHBOARD{" "}
            </h2>

            <button
              onClick={() => navigate("/challenger/uploadedgames")}
              className="bg-wb-100 px-[40px] h-[43px] md:h-[96px] font-Archivo_Regular text-[12px] mt-[24px] md:text-[20px] rounded-[16px] w-fit"
            >
              VIEW PENDING GAMES
            </button>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-[24px]">
            <div
              className="w-full p-[4px] rounded-[5px]"
              style={{
                background: "linear-gradient(90deg, #032449, #0B77F0)",
              }}
            >
              <div
                className="font-Inter_Regular w-full bg-black  py-[24px] px-[24px] "
                style={{
                  boxShadow:
        ...
        <table className="w- max-w-[100vw]">
            ...
            <tr className="font-droid t w-[30%] text-[20px] md:text-[36px] leading-[36px] text-blue-80">
              <th>
                <div
                  ...
                  className="pb-[4px] "
                >
                  <p className="bg-black py-[32px] md:px-[24px] text-center md:text-left">
                    {" "}
                    Game
                  </p>
                </div>
              ...
              <th className="w-[40vw]">
                <div
                  ...
                  className="pb-[4px] "
                >
                  <p className="bg-black py-[32px] nd:px-[24px] text-center md:text-left">
                    {" "}
                    Voting
                  </p>
                </div>
              ...
              <th>
                <div
                  ...
                  className="pb-[4px] "
                >
                  <p className="bg-black py-[32px] md:px-[24px] hidden md:block">
                    {" "}
                    STATUS
                  </p>
                </div>
          ...
          <tbody>
            {loading ? (
              ...
              data &&
              data?.gamesCreated.fetchRes?.map((item: any, i: number) => (
                <tr key={i}>
...
const Game = ({ image, address, title, approve, rejectionMessage }: any) => {
  ...
  const getJudges = async () => {
    ...
    const totalApprovedGameVotes = Number(
      await mmContract.getGameVotes(address)
    );
    const minVotes = Number(await mmContract.getMinVote());
    const totalVotesForGame = Number(
      await mmContract.getVotesForGames(address)
    );
        ...
        <div className="rounded-full overflow-hidden border mr-2 w-[50px] h-[50px] ">
          <img
            src={
              image.includes("http")
                ? image
                : "https://mentalmaze-game.infura-ipfs.io/ipfs/" + image
            }
      ...
      <td>
        ...
        <div className="bg-disppaprove h-[8px] w-full rounded-[20px]">
          <div
            style={{
              width: `${
                votingStat?.totalVotesForGame > 0
                  ? (votingStat?.totalApprovedGameVotes /
                      votingStat?.totalVotesForGame) *
                    100
                  : 50
              }%`,
