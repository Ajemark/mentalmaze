...
const ChallengerGame = () => {
  ...
  const getPendingGame = () => {
    ...
    fetch(
      ...
      }/api/game/get-all-pending-games?pageNumber=1&pageSize=20`,
      requestOptions
    )
      ...
      .then((result) => {
        if (result.data) {
          ...
          const data = result.data.response.filter(
            (game: any) => game.id == id
          )[0];
          setCurGame(data);
  ...
  const approveGameOnDB = () => {
    ...
    fetch(
      `${
        import.meta.env.VITE_REACT_APP_BASE_URL
      }/api/game/approve-pending-games`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
  ...
  const sendTx = async (type: String) => {
    ...
    try {
      ...
      if (msg.includes("you are only allowed to vote ones"))
        msg = "You Are Only Allowed To Vote Once!";
      setErrorMessage(msg);
      console.log(error);
    }
  };
  ...
  return (
    <div>
      {loading ? (
        ...
        <div className="backdrop-blur-sm flex flex-col relative items-center mt-[96px] md:mt-[104px]  md:border-t-solid border-t-2 border-l-2 border-1 border-blue-50 px-[20px] ">
            ...
            <div className={`relative flex  `}>
              {curGame?.question?.map((_: any, i: number) => (
                <div key={i}>
                  <div
                    key={i}
                    className={`relative text-white rounded-[8px] w-[27px] h-[30px] text-[12px] md:text-base md:w-[48px] md:h-[40px]   border-solid shadow-lg`}
                    style={{
                      background:
                  ...
                  >
                    <button
                      ...
                      className={` w-full h-full rounded-[10px] ${
                        i == curQuestion ? "bg-blue-70" : "bg-blue-100"
                      }`}
                    >
                      {i + 1}
                    </button>
          ...
          {curGame && Object.keys(curGame).length > 1 && (
            <div className="w-full px-[16px] md:px-[52px] ">
              <h1 className="font-droid text-white text-center text-[16px] md:text-[32px]    w-full mt-[40px]">
              ...
              <div className="mt-[32px] md:mt-[72px] flex flex-col items-center gap-[32px]">
                {curGame?.question[curQuestion].image.includes("http") ? (
                  ...
                  <p className="font-droid text-white text-center text-[16px] md:text-[22px]   w-full mt-[40px]">
                    {curGame?.question[curQuestion].image}
                  </p>
                )}
              </div>
              <div className="flex w-full mt-10 justify-center flex-wrap gap-[16px]">
                {curGame?.question[curQuestion].options.map(
                  (option: any, index: any) => {
                    return (
                      <div
                        key={index}
                        className="p-3 bg-[inherit] items-center flex justify-center border-blue-main border-[3px] rounded-[8px] border-solid h-[60px] w-[calc(50% - 16px)] text-white"
                      >
                        {option.toUpperCase()}
                      </div>
                    );
              ...
              <div className="flex flex-col justify-center mx-auto w-fit">
                <p className="text-white text-[16px] font-Archivo_Regular font-[400] leading-[17.41px] mt-[32px] mx-auto">
                  Correct Answer
                </p>
          ...
          >
            <div className="text-white font-Archivo_Regular text-[20px]  bg-wb-100  rounded-[24px]  pb-[55px]">
              <p className="mx-auto text-center font-droid text-[20px] py-[16px] md:text-[32px] leading-[37.78px] pt-[16px] pb-[32px] border-b-[4px] border-b-[#0B77F0] w-full">
                {curGame?.title}
              </p>
              <p className="px-[40px] md:px-[42px] pt-[52px] leading-[21.76px]]">
                Math Puzzle: Unleash your inner whiz, where speed and accuracy
                meld together harmoniously, paving the path to triump and
                unlocking thesecrets of mathematical mastery!
              </p>
            </div>
          </div>
        </div>
